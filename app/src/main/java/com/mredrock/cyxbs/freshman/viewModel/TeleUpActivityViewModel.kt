...
class TeleUpActivityViewModel : BaseViewModel() {

    private var isClick = false

    //状态栏沉浸式
    fun visibleState(activity: TelePhoneUPActivity) {
    ...
    //edit的控制
    fun doToEdit(activity: TelePhoneUPActivity) {
        activity.et_tele_logup.addTextChangedListener(object : TextWatcher {
            ...
            override fun beforeTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
                if (p0.toString() == "") {
                    activity.tele_cv_logup.visibility = View.INVISIBLE
                }
            }
            ...
            override fun onTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
                if (p0.toString() == "") {
                    ...
                    activity.tele_cv_logup.visibility = View.VISIBLE
                }
            }
    ...
    //验证码发送
    fun sendDefine(activity: TelePhoneUPActivity) {
        activity.sl_send_define_logup.setOnClickListener {
            if (isClick == false) {
                activity.tv_send_define_logup.setTextColor(Color.parseColor("#808080"))
                isClick = true
                if (activity.et_tele_logup.text.toString() != "") {
                    val avUser = AVUser()
                    avUser.mobilePhoneNumber = "+86${activity.et_tele_logup.text}"
                    AVUser.requestLoginSmsCodeInBackground("+86${activity.et_tele_logup.text}")
                        .blockingSubscribe()
                } else {
                    Toast.makeText(activity, "手机号不能为空", Toast.LENGTH_SHORT).show()
                    isClick = false
                    activity.tv_send_define_logup.setTextColor(Color.parseColor("#000000"))
                }
            }
        }
    }
    ...
    //注册
    fun writeDefine(activity: TelePhoneUPActivity) {
        activity.asv_tele_logup.setOnClickListener {
            if (activity.et_define_logup.text.toString() != "") {
                AVUser.signUpOrLoginByMobilePhoneInBackground(
                    "+86${activity.et_tele_logup.text}",
                    activity.et_define_logup.text.toString()
                )
